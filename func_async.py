...
def check_tsi_confirmation(candles: List[List[str]],
    ...
    """
    ИСПРАВЛЕННАЯ ЛОГИКА: Проверка подтверждения сигнала с помощью TSI
    Проверяет пересечения на последних 5 свечах для большей вероятности поймать сигнал
    ...
    if len(tsi_data['tsi']) < 6:  # Нужно минимум 6 точек для проверки 5 пересечений
        return False

    tsi_values = tsi_data['tsi']
    signal_values = tsi_data['signal']

    # Проверяем последние 5 свечей (индексы -5, -4, -3, -2, -1)
    for i in range(-5, 0):  # Проверяем 5 последних свечей
        ...
        # Проверяем что у нас есть данные для обоих индексов
        if abs(prev_idx) > len(tsi_values):
            continue
        ...
        if signal_type == 'LONG':
            ...
            # Или быть выше сигнальной линии с восходящим трендом
            if (prev_tsi <= prev_signal and current_tsi > current_signal) or \
                    (current_tsi > current_signal and current_tsi > prev_tsi):
                return True
        elif signal_type == 'SHORT':
            # Для SHORT: TSI должен пересечь сигнальную линию сверху вниз
            # Или быть ниже сигнальной линии с нисходящим трендом
            if (prev_tsi >= prev_signal and current_tsi < current_signal) or \
                    (current_tsi < current_signal and current_tsi < prev_tsi):
                return True

    # Дополнительная проверка: текущее состояние TSI
    current_tsi = tsi_values[-1]
    current_signal = signal_values[-1]

    if signal_type == 'LONG' and current_tsi > current_signal:
        return True
    elif signal_type == 'SHORT' and current_tsi < current_signal:
        return True
...
def analyze_last_candle_with_tsi(bybit_candles: List[List[str]],
    ...
    ИСПРАВЛЕННАЯ ЛОГИКА: проверяет пересечения на последних 5 свечах
    ...
    if len(tsi_data['tsi']) < 6:  # Нужно минимум 6 точек
        return 'NO_SIGNAL'

    tsi_values = tsi_data['tsi']
    signal_values = tsi_data['signal']

    # Проверяем последние 5 пересечений
    for i in range(-5, 0):  # Проверяем 5 последних свечей
        if len(tsi_values) + i < 1:  # Защита от выхода за границы
            continue
    ...
    if current_tsi > current_signal and current_tsi > tsi_values[-2]:
        return 'LONG'
    elif current_tsi < current_signal and current_tsi < tsi_values[-2]:
        return 'SHORT'
